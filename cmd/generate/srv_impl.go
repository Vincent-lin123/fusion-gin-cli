...
import (
	...
	"{{.PkgName}}/model/gormx/model"
	"{{.PkgName}}/schema"
	"{{.PkgName}}/lib/errors"
	"{{.PkgName}}/lib/util/uuid"
...
func getSrvImplFileName(dir, name string) string {
	fullname := fmt.Sprintf("%s/service/%s.go", dir, util.ToLowerUnderlinedNamer(name))
	return fullname
}

func genSrvImpl(ctx context.Context, pkgName, dir, name, comment string) error {
	...
	fullname := getSrvImplFileName(dir, name)
